...
fun countFirst(s: Any): Int {
    val firstChar: Char? = (s as? CharSequence)?.firstOrNull()
    if (firstChar != null)
...
fun Int.isNotfit(): Unit{
    val a= this +1
}

fun Int.isNotFit2(): Boolean = this > 2
...
fun main() {
    ...
    val keys: Iterable<Char> = 'a'..'f'
    val map = keys.associateWith { it.toString().repeat(5).capitalize() }
    map.forEach { println(it) }

    map.forEach { it: Map.Entry<Char,String> -> println(it) }
